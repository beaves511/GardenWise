...
# --- CONSTANTS (Naming convention: CAPS_WITH_UNDERSCORES) ---

SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_ANON_KEY = os.getenv("SUPABASE_PUBLIC_KEY")
# NOTE: The Supabase Service Role Key is used for high-privilege operations
# like inserting a profile.
...
def create_user_profile_record(user_id: str, email: str) -> bool:
    """
    Inserts a new record into the 'profiles' table using the Service Key.

    This function should be called immediately after a successful user signup.
    """
    headers = {
    ...
    try:
        ...
        if response.status_code >= 400:
            print(f"ERROR: Failed to create profile for {email}. "
...
def sign_up(email: str, password: str) -> Tuple[Dict[str, Any], int]:
    """
    Registers a new user with Supabase Auth and creates a database profile.

    Returns:
        A tuple containing the response data (dict) and HTTP status code (int).
    """
    ...
    try:
        ...
        if response.status_code == 200 and 'user' in auth_data:
            ...
            # CRITICAL STEP: Create the profile record
...
def sign_in(email: str, password: str) -> Tuple[Dict[str, Any], int]:
    """
